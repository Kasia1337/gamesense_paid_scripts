...
watermark = function() -- watermark function
  ...
  local watermark_string = string.format("Welcome %s to Tonka.lua | Build: %s","User","Debug")
...
intro_animation = function()
    ...
    if should_animate == "should_intro" then
        ...
        renderer.text(X / 2, y , 255, 255, 255, alpha,"c",0,string.format("User - %s","User"))
...
function aim_hit(e)
    ...
    print(string.format("hit: %s | predicted hitbox: %s / hitbox: %s | predicted damage: %s / damage: %s | hc: %s | bt_ticks: %s",name,string.lower(predicted_group),string.lower(hitbox),predicted_damage,damage,hc,backtrack))
...
zeus_esp = function()
    ...
    for i = 1, #playersz do
        ...
        elseif has_zeus and weapon.name == "Zeus x27" then
            --renderer.text((bounding_box[1] + bounding_box[3]) / 2 - 1,bounding_box[4] + 20,255,0,0,255,"-c",0,"HEdHEH HAW")
