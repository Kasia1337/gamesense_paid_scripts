...
local ethereal.lua = {
    master              = ui_new_checkbox(tab, container, '[Initiate] ethereal.lua'),
    ...
    visual = {
        crosshair = {
            master = ui_new_checkbox(tab, container, '[ethereal.lua] Indicators')
        },
...
local function aa_on_use(cmd)
	...
	if contains(ui_get(ethereal.lua.aa.extra), 'Legit anti-aim') then 
		local plocal = entity.get_local_player()
		...
		local trynna_plant = on_bombsite ~= 0 and team_num == 2 and has_bomb and not ui_get(ethereal.lua.aa.legit)
        ...
        client.set_event_callback('paint_ui', function()
            ...
            for f = #data, 1, -1 do
                ...
                if _data.time < 0 then
                    ...
                    local color = {ui_get(ethereal.lua.visual.colorpicker.desynclog)}

                    local text_left = "[ethereal.lua] "
...
notify:paint(13, "Initialized ethereal.lua...")
...
local function COMMAND_HANDLER(e, cmd)
    ...
    if not ui_get(ethereal.lua.master) then return end
...
local function custom_aa()
    if not contains(ui_get(ethereal.lua.aa.extra), 'Custom anti-aim') then return end
    ...
    if ui_get(ethereal.lua.aa.legit) then 
...
function antiaim(cmd)
    ...
    if not contains(ui_get(ethereal.lua.aa.extra), "Custom anti-aim") then 
    ...
    if var.best_enemy ~= nil then 
        ...
        if #target > 0 then
            ...
            if ui_get(ethereal.lua.aa.roll_dd) == 'Side Roll' then
                ...
                if ui_get(ethereal.lua.aa.roll_ab) and ui_get(ethereal.lua.aa.roll_cb) then 
            ...
            elseif ui_get(ethereal.lua.aa.roll_dd) == 'Normal' then 
                if ui_get(ethereal.lua.aa.roll_ab) and ui_get(ethereal.lua.aa.roll_cb) then 
                    cmd.roll = 42
                    ui_set(ref.aa.fakeyaw, 60)                
                    ui_set(ref.aa.freestand_bodyyaw, false)               
        ...
        if states.crouch() then 
            cmd.roll = contains(ui_get(ethereal.lua.aa.rollstates), 'Ducking') and 42 or 0
        elseif ui_get(ref.aa.slowwalk[1]) and ui_get(ref.aa.slowwalk[2]) and (not states.air()) then 
            cmd.roll = contains(ui_get(ethereal.lua.aa.rollstates), 'Slowwalking') and 42 or 0
        elseif states.air() then 
            cmd.roll = contains(ui_get(ethereal.lua.aa.rollstates), 'Jumping') and 42 or 0
        elseif states.moving() then 
            cmd.roll = contains(ui_get(ethereal.lua.aa.rollstates), 'Moving') and 42 or 0
        elseif states.standing() then 
            cmd.roll = contains(ui_get(ethereal.lua.aa.rollstates), 'Standing') and 42 or 0
        end

        if contains(ui_get(ethereal.lua.aa.rollstates), 'Low Velocity') then 
        ...
        if (ui_get(ethereal.lua.aa.roll_ab)) then return end
        ...
        if entity.get_prop(entity.get_local_player(), "m_bWarmupPeriod") then 
            ...
            if not (contains(ui_get(ethereal.lua.aa.jittercondition), "Always") or contains(ui_get(ethereal.lua.aa.jittercondition), "Vulnerable")) then
                if states.crouch() and (not states.air()) then --Crouch
                    ui_set(ref.aa.fakeyaw, 60)                
                    ui_set(ref.aa.freestand_bodyyaw, false)               
                    ui_set(ref.aa.edgeyaw, false)               
            ...
            elseif contains(ui_get(ethereal.lua.aa.jittercondition), "Always") then
        ...
        if contains(ui_get(ethereal.lua.aa.jittercondition), "Vulnerable") then
    ...
    if contains(ui_get(ethereal.lua.aa.jittercondition), "On Playerstate") then
            if states.crouch() and (not states.air()) then 
                if contains(ui_get(ethereal.lua.aa.playerstates), 'Ducking') then 
                    ui_set(ref.aa.fakeyaw, 60)                
                    ui_set(ref.aa.freestand_bodyyaw, false)               
                    ui_set(ref.aa.edgeyaw, false)               
                    ui_set(ref.aa.pitch, 'Default')                
            ...
            elseif ui_get(ref.aa.slowwalk[1]) and ui_get(ref.aa.slowwalk[2]) and (not states.air()) then
                if contains(ui_get(ethereal.lua.aa.playerstates), 'Slowwalking') then
                    ui_set(ref.aa.fakeyaw, 60)              
                    ui_set(ref.aa.freestand_bodyyaw, false)
                    ui_set(ref.aa.edgeyaw, false)
                    ui_set(ref.aa.pitch, 'Default')                
                    ...
                    ui_set(ref.aa.yaw_jitter[2], 67)
                    ui_set(ref.aa.body_yaw[1], 'Jitter')
                elseif not contains(ui_get(ethereal.lua.aa.playerstates), 'Slowwalking') then
                    ui_set(ref.aa.fakeyaw, 60)                
                    ui_set(ref.aa.freestand_bodyyaw, false)               
                    ui_set(ref.aa.edgeyaw, false)               
                    ui_set(ref.aa.pitch, 'Default')                
            ...
            elseif states.air() then
                if contains(ui_get(ethereal.lua.aa.playerstates), 'Jumping') then
                    if rand == 0 then 
                        ui_set(ref.aa.fakeyaw, 60)     
                    elseif rand == 1 then 
                        ui_set(ref.aa.fakeyaw, 42) 
                    ...
                    ui_set(ref.aa.body_yaw[1], 'Jitter')
                elseif not contains(ui_get(ethereal.lua.aa.playerstates), 'Jumping') then
                    ui_set(ref.aa.fakeyaw, 60)         
                    ui_set(ref.aa.freestand_bodyyaw, false)               
                    ui_set(ref.aa.edgeyaw, false)               
                    ui_set(ref.aa.pitch, 'Default')                
            ...
            elseif states.moving() and not states.crouch() then
                if contains(ui_get(ethereal.lua.aa.playerstates), 'Moving') then
                    if (oldtime+0 <= globals.realtime()) then
                        ui_set(ref.aa.fakeyaw, cycle_table(fakeyaw)) 
                        oldtime = globals.realtime()
                    end   
                    ...
                    ui_set(ref.aa.yaw_jitter[2], 82)    
                    ui_set(ref.aa.body_yaw[1], 'Jitter')
                elseif not contains(ui_get(ethereal.lua.aa.playerstates), 'Moving') then
                    ui_set(ref.aa.fakeyaw, 60)       
                    ui_set(ref.aa.freestand_bodyyaw, false)               
                    ui_set(ref.aa.edgeyaw, false)               
                    ui_set(ref.aa.pitch, 'Default')                
            ...
            elseif states.standing() then
                if contains(ui_get(ethereal.lua.aa.playerstates), 'Standing') then
                    ui_set(ref.aa.fakeyaw, 60)        
                    ui_set(ref.aa.freestand_bodyyaw, false)               
                    ui_set(ref.aa.edgeyaw, false) 
                    ui_set(ref.aa.pitch, 'Default')                
                    ...
                    ui_set(ref.aa.yaw_jitter[2], 8)
                    ui_set(ref.aa.body_yaw[1], 'Jitter')
                elseif not contains(ui_get(ethereal.lua.aa.playerstates), 'Standing') then
                    ui_set(ref.aa.fakeyaw, 60)        
                    ui_set(ref.aa.freestand_bodyyaw, false)               
                    ui_set(ref.aa.edgeyaw, false) 
                    ui_set(ref.aa.pitch, 'Default')                
        ...
        if contains(ui_get(ethereal.lua.aa.jittercondition), "On Exploit") then 
    ...
    if contains(ui_get(ethereal.lua.aa.jittercondition), "Enemy Dormant") and var.best_enemy == nil then
        ui_set(ref.aa.fakeyaw, 60) 
        ui_set(ref.aa.pitch, 'Default') 
        ui_set(ref.aa.yaw[1], 180)                
        ...
        ui_set(ref.aa.yaw_jitter[2], 65)
        ui_set(ref.aa.body_yaw[1], 'Jitter')
    elseif not contains(ui_get(ethereal.lua.aa.jittercondition), "Enemy Dormant") and var.best_enemy == nil then
        ui_set(ref.aa.fakeyaw, 60)         
        ui_set(ref.aa.freestand_bodyyaw, false)               
        ui_set(ref.aa.edgeyaw, false)               
        ui_set(ref.aa.pitch, 'Default')                
    ...
    if ui_get(ethereal.lua.aa.legit) and contains(ui_get(ethereal.lua.aa.extra), 'Legit anti-aim') then 
        ui_set(ref.aa.fakeyaw, 60)         
        ui_set(ref.aa.freestand_bodyyaw, false)               
        ui_set(ref.aa.edgeyaw, false)               
...
client.set_event_callback('bullet_impact', function(c)
    ...
    if entity.is_alive(entity.get_local_player()) and not skip then
        ...
        if not entity.is_dormant(ent) and entity.is_enemy(ent) then
            ...
            if math.abs(delta) < ui_get(ethereal.lua.aa.bullet_detection) / 1.6666667 then
                ab = ab + 1
                flip = true
            end
        end
    ...
    if flip then
        if ui_get(ethereal.lua.aa.antibrute_type) == 'Default' then
            ui_set(ref.aa.body_yaw[2], default[(ab % 2) + 1])
        elseif ui_get(ethereal.lua.aa.antibrute_type) == 'Aggressive' then
...
client.set_event_callback('setup_command', function()
    if not ui_get(ethereal.lua.master) then return end
    if not ui_get(ethereal.lua.fakelag.dynamic) then return end
    ...
    if ui_get(ethereal.lua.fakelag.dynamic_type) == 'Advanced' then 
    ...
    elseif ui_get(ethereal.lua.fakelag.dynamic_type) == 'Default' then 
        ui_set(ref.fakelag.variance, 20)
...
client.set_event_callback("pre_render", function()
    ...
    if contains(ui_get(ethereal.lua.aa.extra), 'Leg Breaker') then
    ...
    if contains(ui_get(ethereal.lua.aa.extra), 'Leg Breaker') then
        ...
        if randome > ui_get(ethereal.lua.aa.legs) then
...
local function hotkey_setup()
    if not ui_get(ethereal.lua.master) then return end
        ...
        ui_set(ethereal.lua.dir.manualleft, "On hotkey")
        ui_set(ethereal.lua.dir.manualright, "On hotkey")

    if ui_get(ethereal.lua.dir.free) and ui_get(ethereal.lua.dir.customdir) then
    ...
    if ui_get(ethereal.lua.dir.edge) and ui_get(ethereal.lua.dir.customdir) then
        ui_set(ref.aa.edgeyaw, true)
    else
        ui_set(ref.aa.edgeyaw, false)
    end

    if ui_get(ethereal.lua.dir.customdir) then
        if ui_get(ethereal.lua.dir.manualback) then
            mode = "back"
        elseif ui_get(ethereal.lua.dir.manualleft) and leftoff then
            if mode == "left" then
                mode = "back"
            else
                mode = "left"
            ...
            leftoff = false
        elseif ui_get(ethereal.lua.dir.manualright) and rightoff then
            if mode == "right" then
                mode = "back"
            else
                mode = "right"
        ...
        if ui_get(ethereal.lua.dir.manualleft) == false then
            leftoff = true
        end
        if ui_get(ethereal.lua.dir.manualright) == false then
            rightoff = true
        end
        if mode == "back" then
...
local function misc_features()
    if ui_get(ethereal.lua.master) and ui_get(ethereal.lua.misc.dt.doubletapboostcheck) then
        local ping = math.floor(client.latency() * 1000)
        if ui_get(ethereal.lua.misc.dt.doubletapboostdrop) == "Default" then
            ui_set(ref.misc.max_ticks, 16)
        elseif ui_get(ethereal.lua.misc.dt.doubletapboostdrop) == "Safe" then
            ui_set(ref.misc.max_ticks, 15)
        elseif ui_get(ethereal.lua.misc.dt.doubletapboostdrop) == "Dangerous" then
            ui_set(ref.misc.max_ticks, 17)
        end
        if ping <= 55 and ui_get(ethereal.lua.misc.dt.doubletapboostdrop) == "Adaptive" then
            ui_set(ref.misc.max_ticks, 17)
        elseif ping >= 55 and ui_get(ethereal.lua.misc.dt.doubletapboostdrop) == "Adaptive" then
            ui_set(ref.misc.max_ticks, 15)
        end
        if ui_get(ethereal.lua.misc.dt.doubletapboostdrop) == "Custom" then
            ui_set(ref.misc.max_ticks, ui_get(ethereal.lua.misc.dt.doubletapboostcustom))
        end
    end
end
...
client.set_event_callback('paint', function()
    ...
    if ui_get(ethereal.lua.misc.sunsetmode) then
...
local killtable = {
    ...
    "for you this 1's. for me these heads, by ethereal.lua (shoppy.gg/@rite)",
    "Sent to hell, by ethereal.lua (shoppy.gg/@rite)",
...
local function on_player_death(e)
	if not ui_get(ethereal.lua.misc.killsay) then return	end
...
local function crosshair_indicators()
    ...
    local hotkeyc = { ui_get(ethereal.lua.visual.debug.debug_color)  }
    ...
    --color pickers
    local namec = {ui_get(ethereal.lua.visual.colorpicker.name_color)}
    local state = {ui_get(ethereal.lua.visual.colorpicker.playerstate_color)}
    local exploitc = {ui_get(ethereal.lua.visual.colorpicker.exploit_color)}
    local arrowc   = {ui_get(ethereal.lua.visual.colorpicker.arrow_color)}
    ...
    if ui_get(ethereal.lua.visual.crosshair.master) and contains(ui_get(ethereal.lua.visual.indicatortype), "Crosshair") and not is_scoped then
        renderer.text(x/2, y/2 + 30, namec[1], namec[2], namec[3], 255, 'c-', nil, 'ethereal.lua')
        if states.crouch() and (not states.air()) then --Crouch
    ...
    elseif ui_get(ethereal.lua.visual.crosshair.master) and contains(ui_get(ethereal.lua.visual.indicatortype), "Crosshair") and is_scoped then 
        renderer.text(x/2, y/2 + 30, namec[1], namec[2], namec[3], 255, 'c-', nil, 'ethereal.lua')
        if states.crouch() and (not states.air()) then --Crouch
            renderer.text(x/2, y/2 + 40, state[1], state[2], state[3], 255, 'c-', nil, 'DUCKING')
        elseif ui_get(ref.aa.slowwalk[1]) and ui_get(ref.aa.slowwalk[2]) and (not states.air()) then -- Slow motion
    ...
    if contains(ui_get(ethereal.lua.visual.indicatortype), "Debug Panel") then
        container_glow(x/200,  y/2 + 15, 165, 50, hotkeyc[1], hotkeyc[2], hotkeyc[3], hotkeyc[4], 1.2, hotkeyc[1], hotkeyc[2], hotkeyc[3])
        renderer.text(x/200 + 5,  y/2 + 20, 255, 255, 255, 255, "Light", 0, ">> ethereal.lua")
...
client.set_event_callback("aim_hit", function (e)
    ...
    if not contains(ui_get(ethereal.lua.visual.logs), 'On Screen') then return end
...
client.set_event_callback("aim_hit", function(e)
    if not contains(ui_get(ethereal.lua.visual.logs), 'Chat') then return end
    bt = globals.tickcount() - e.tick -- e.backtrack
    backtrack = bt
    local group = hitbox [e.hitgroup]
    local previous_health = entity.get_prop(e.target, "m_iHealth")

    chat.print(string.format('{red}[ethereal.lua]{white} Hit {red}%s{white} in the {red}%s{white} ({red}%d{white} health remaining, BT: {red}%s{white})', entity.get_player_name(e.target), group, previous_health, backtrack))
end)

client.set_event_callback("aim_miss", function (e)
    if not contains(ui_get(ethereal.lua.visual.logs), 'Chat') then return end
    local group = hitbox [e.hitgroup]
    chat.print(string.format('{red}[ethereal.lua]{white} Missed {red}%s{white} in the {red}%s{white} due to {red}%s{white}', entity.get_player_name(e.target), group, e.reason))
end)
...
local function ON_BULLET_IMPACT(e)
    ...
    if entity.is_enemy(shooter_idx) and not entity.is_dormant(shooter_idx) then
		...
		if math.abs(dist) <= ui_get(ethereal.lua.aa.bullet_detection) then
...
client.set_event_callback('aim_miss', function(e)
    if not contains(ui_get(ethereal.lua.visual.logs), "Console") then return end
    local group = hitbox [e.hitgroup]
    local empty_string = (string.format('Missed %s in the %s due to %s', entity.get_player_name(e.target), group, e.reason)) 
        ...
        for k, v in pairs(list) do
            ...
            if type(mode) == "table" then
                for j in pairs(mode) do
                    ...
                    ui.set_visible(mode[j], active and visible and set_element and ui_get(ethereal.lua.tabtype) == "Anti-aim" )
                end
            end
        end
    end
    
    local state = ui_get(ethereal.lua.master)
    local region = ui_get(ethereal.lua.tabtype)
    local customaa, roll = contains(ui_get(ethereal.lua.aa.extra), 'Custom anti-aim'), contains(ui_get(ethereal.lua.aa.extra), 'Roll on key')

    multi_exec(ui.set_visible, {
        [ethereal.lua.tabtype] = state,

        [ethereal.lua.aa.jittercondition]                      = state and region == "Anti-aim" and not customaa,
        [ethereal.lua.aa.roll_ab]                              = state and region == "Anti-aim" and roll,
        [ethereal.lua.aa.roll_cb]                              = state and region == "Anti-aim" and roll,
        [ethereal.lua.aa.antibrute_type]                       = state and region == "Anti-aim" and not customaa,
        [ethereal.lua.aa.bullet_detection]                     = state and region == "Anti-aim",
        [ethereal.lua.aa.roll_dd]                              = state and region == "Anti-aim" and ui_get(ethereal.lua.aa.roll_cb) and roll,
        [ethereal.lua.aa.playerstates]                         = state and region == "Anti-aim" and contains(ui_get(ethereal.lua.aa.jittercondition), 'On Playerstate') and not customaa,
        [ethereal.lua.aa.extra]                                = state and region == "Anti-aim",
        [ethereal.lua.aa.rollstates]                           = state and region == "Anti-aim",
        [ethereal.lua.aa.legit]                                = state and region == 'Anti-aim' and contains(ui_get(ethereal.lua.aa.extra), 'Legit anti-aim'),
        [ethereal.lua.aa.legs]                                 = state and region == 'Anti-aim' and contains(ui_get(ethereal.lua.aa.extra), 'Leg Breaker'),
        ...
        [ethereal.lua.fakelag.dynamic]                         = state and region == 'Rage',
        [ethereal.lua.fakelag.dynamic_type]                    = state and region == 'Rage' and ui_get(ethereal.lua.fakelag.dynamic),
        [ethereal.lua.misc.dt.doubletapboostcheck]             = state and region == "Rage",
        [ethereal.lua.misc.dt.doubletapboostdrop]              = state and region == "Rage" and ui_get(ethereal.lua.misc.dt.doubletapboostcheck),
        [ethereal.lua.misc.dt.doubletapboostcustom]            = state and region == "Rage" and ui_get(ethereal.lua.misc.dt.doubletapboostcheck) and ui_get(ethereal.lua.misc.dt.doubletapboostdrop) == 'Custom', 
        [ethereal.lua.visual.logs]                             = state and region == "Rage",                         

        [ethereal.lua.visual.crosshair.master]                 = state and region == "Visuals",
        [ethereal.lua.visual.indicatortype]                    = state and region == "Visuals" and ui_get(ethereal.lua.visual.crosshair.master),
        [ethereal.lua.visual.debug.debug_label]                = state and region == "Visuals" and contains(ui_get(ethereal.lua.visual.indicatortype), "Debug Panel") and ui_get(ethereal.lua.visual.crosshair.master),
        [ethereal.lua.visual.debug.debug_color]                = state and region == "Visuals" and contains(ui_get(ethereal.lua.visual.indicatortype), "Debug Panel") and ui_get(ethereal.lua.visual.crosshair.master),
        [ethereal.lua.visual.colorpicker.desynclog]            = state and region == "Visuals",
        [ethereal.lua.visual.colorpicker.desync_label]         = state and region == "Visuals", 
        [ethereal.lua.visual.colorpicker.label]                = state and region == "Visuals" and contains(ui_get(ethereal.lua.visual.indicatortype), "Crosshair") and ui_get(ethereal.lua.visual.crosshair.master),
        [ethereal.lua.visual.colorpicker.playerstate]          = state and region == "Visuals" and contains(ui_get(ethereal.lua.visual.indicatortype), "Crosshair") and ui_get(ethereal.lua.visual.crosshair.master),
        [ethereal.lua.visual.colorpicker.name]                 = state and region == "Visuals" and contains(ui_get(ethereal.lua.visual.indicatortype), "Crosshair") and ui_get(ethereal.lua.visual.crosshair.master),
        [ethereal.lua.visual.colorpicker.playerstate_color]    = state and region == "Visuals" and contains(ui_get(ethereal.lua.visual.indicatortype), "Crosshair") and ui_get(ethereal.lua.visual.crosshair.master),
        [ethereal.lua.visual.colorpicker.name_color]           = state and region == "Visuals" and contains(ui_get(ethereal.lua.visual.indicatortype), "Crosshair") and ui_get(ethereal.lua.visual.crosshair.master),
        [ethereal.lua.visual.colorpicker.exploit]              = state and region == "Visuals" and contains(ui_get(ethereal.lua.visual.indicatortype), "Crosshair") and ui_get(ethereal.lua.visual.crosshair.master),
        [ethereal.lua.visual.colorpicker.exploit_color]        = state and region == "Visuals" and contains(ui_get(ethereal.lua.visual.indicatortype), "Crosshair") and ui_get(ethereal.lua.visual.crosshair.master),
        [ethereal.lua.misc.sunsetmode]                         = state and region == "Visuals",
        [ethereal.lua.visual.colorpicker.arrow_color]          = state and region == "Visuals",
        [ethereal.lua.visual.colorpicker.arrow]                = state and region == "Visuals",

        [ethereal.lua.dir.customdir]                           = state and region == "Miscellaneous",
        [ethereal.lua.dir.free]                                = state and region == "Miscellaneous" and ui_get(ethereal.lua.dir.customdir),
        [ethereal.lua.dir.edge]                                = state and region == "Miscellaneous" and ui_get(ethereal.lua.dir.customdir),
        [ethereal.lua.dir.manualback]                          = state and region == "Miscellaneous" and ui_get(ethereal.lua.dir.customdir),
        [ethereal.lua.dir.manualleft]                          = state and region == "Miscellaneous" and ui_get(ethereal.lua.dir.customdir),
        [ethereal.lua.dir.manualright]                         = state and region == "Miscellaneous" and ui_get(ethereal.lua.dir.customdir),
        [ethereal.lua.misc.killsay]                            = state and region == "Miscellaneous",
